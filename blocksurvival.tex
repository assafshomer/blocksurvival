...
Now the attacker, joined by $\gamma$ of the honest are competing with the rest of the honest miners. The probability to win starting from a tie is exactly given by \ref{eq:az}
...
\begin{equation}
S_\gamma(q)=T(q)\cdot a_0(q_{eff})=
\begin{cases}
2q\cdot\dfrac{q(1-\gamma)+\gamma}{(1-q)(1-\gamma)} & q_{eff}\in [0,\dfrac{1}{2}] \\ 
2q & q_{eff}\in [\dfrac{1}{2},1] \\ 
\end{cases}
\end{equation}
