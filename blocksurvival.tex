...
\section{The Strategy Phase Space}

Now we can chart the strategy phase space parametrized by $\gamma,q$. The calculations are left for appendix \ref{app:phasespace}, and result in the following chart:

\begin{figure}[qgammaphasespace]
\centering
\includegraphics[width=150mm]{simpleqgammaphasespace.png}
\caption{The value of $q_c$ and $q_b$ as a function of $\gamma$.}
...
\section{The $\gamma, q$ phase space calculation}\label{app:phasespace}
Let us explore the $\gamma, q$ phase space $[0,1]\times[0,1]$ and map in each area which is the most beneficial mining strategy:
\begin{itemize}
\item Standard (a.k.a Honest)
\item Type I
\item Type 0
\end{itemize}
...
\begin{itemize}
\item Equation \ref{eqn:qnot} tells us that in the regime $q>q_0=1-\frac{1}{\sqrt{2}}$ the Type 1 strategy is more beneficial than the standard strategy.
...
The rest of phase space requires a little more tedious analysis:

\subsubsection{The regime $\gamma_c=1-\frac{2\sqrt{2}}{3}\sim 0.0572\dots\leq \gamma \leq \frac{1}{17}\sim 0.0588\dots$ }

In this region we learn from equation \ref{eqn:type0over1gammaregimes} that Type 0 is better than Type 1 only if $q<q_-(\gamma)$ or $q>q_+(\gamma)$. By definition of $\gamma_c$ (equation \ref{eqn:qbornot}) $q_b(\gamma)<q_0$. This, means that as long as $\q_b(\gamma)\leq q \leq q_0$ only Type 0 is better than the standard strategy and so in this regime Type 0 is the most beneficial strategy. If $q<q_b(\gamma)$ the standard strategy wins and finally if $q>q_0$ then both Type 0 and Type 1 are beneficial over the standard strategy and we need to figure out using equation \ref{eqn:type0over1gammaregimes} to figure out which one is the best.
