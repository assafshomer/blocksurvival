...
we can even summarize further by writing

\begin{equation}\label{eqn:0overhonest2}
S_{\gamma}(q)\geq q \Longrightarrow
...
One is to compare how they match against the honest strategy. Namely, for a given $\gamma$ do we first hit the regime where a type 0 or a Type I is beneficial over the honest strategy.
...
Let us explore the $\gamma\in[0,1] \times q\in [0,1]$ phase space and divide it into regions characterized by the most beneficial mining strategy: \textbf{Standard} (a.k.a Honest), \textbf{Type 0} or \textbf{Type I}.

the phase $\gamma, q$ phase space is governed by four functions (really three intersecting curves):
\begin{itemize}
...
In figure \ref{fig:fourfunctions} we plot all 4.
...
However, there is still the option that type 1 wins in this region if we can find a solution where $q_0<q\leq q_+(\gamma)$.
