...
\title{\textbf{The Phase Space of Block-Hiding Mining Strategies}}
...
Bitcoin's consensus mechanism suffers from another weakness. The Bitcoin protocol assumes that miners publish a newly found block immediately and that every node shifts it's mining effort to the top of the block-chain as soon as it is being made aware of a new block. A necessary consequence of Bitcoin's decentralization is that every miner is free to employ alternative mining strategies as it sees fit, in the interest of maximizing it's own profit. If a miner ``breaks the rules", her blocks are in danger of being rejected by the network. However, some wiggle room exists for miners to participate in the Bitcoin network without strictly following the Bitcoin protocol. For example, a miner can choose not to share a newly found block and build a secret branch of the block-tree that she only reveals opportunistically. Such non-traditional strategies were discussed in \cite{Selfish,lear} and were shown to enable miners to increase their profit. In this paper we are interested in finding out if and when  block-hiding strategies give miners a higher probability of success in mining blocks, compared to the standard strategy outlined in the Bitcoin protocol.
...
To that effect we calculate the probability that a block-hiding miner succeeds in replacing a block (and possibly some number of confirmation blocks on top of it) by publishing a secretly mined branch of the block-tree.
...
We start our analysis at a given point in time where the blockchain is of length $L$ and denote the last block mined as $\mathit{B}_L$. As time marches on the Standard miners continue to mine on top of it ($\mathit{B}_{L+1}, \mathit{B}_{L+2}, \dots$) while the block hiding miners are building a separate branch on top of $\mathit{B}_L$ ($\mathit{\tilde{B}}_{L+1}, \mathit{\tilde{B}}_{L+2}, \dots$). This is depicted in figure \ref{fig:blockhidingchain}. The block hiding miners aim to replace the top of the chain mined on top of $\mathit{B}_L$ by using one of the two block hiding strategies.


\section{Calculating the probability of success}

Calculating the probability of success for a block hiding strategy involves two parts. We first calculate the probability that the block tree reaches a situation like the one depicted in figure \ref{fig:blockhidingchain}. Then we multiply this probability with the probability that the block-hiding miner manage to catchup with the main chain. Our analysis follows the one presented in \cite{Doublespend}.
...
The probability $\mathit{a}_{n,m}^{(r)}(q)$ that the block-hiding miner manage to catch-up and overtake the blockchain by at least $r$ blocks, given the situation above\footnote{Namely, that until the moment the main network mines it's $n$th block on top of $\mathit{B}_L$, the block-hiding miner manages to mine $m$ blocks on top of $\mathit{B}_L$ constituting their secret branch.} is given by a Markov chain that depends only on the advantage $z$ of the honest network over the block-hiding miner $z=n-m$, and the parameter $r$. Formally, the chain satisfies the recurrence relation
...
\section{Block hiding Strategy - Type I}
...
\section{Block hiding Strategy - Type 0}
...
\begin{itemize}
\item $q\leq q_b(\gamma)$ In this region neither block hiding strategies are better than the standard strategy, and so a miner will follow the standard strategy.
