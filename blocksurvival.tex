...
Now the attacker, joined by $\gamma$ of the honest are competing with the rest of the honest miners. The probability to win starting from a tie is exactly $a_0(q_{eff})=\dfrac{q_{eff}}{1-q_{eff}}$, where $q_{eff}=q+\gamma p=q+\gamma(1-q)$.

We conclude that 

\begin{equation}
S_\gamma(q)=T(q)\cdot a_0(q_{eff})=2q\cdot\dfrac{q+\gamma(1-q)}{1-(q+\gamma(1-q))}=2q\cdot\dfrac{q(1-\gamma)+\gamma}{(1-q)(1-\gamma)}
\end{equation}
